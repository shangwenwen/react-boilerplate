...
function auth({ email, password }) {
  ...
  return database.users.some(user => user.email === email && user.password === password && user.active)
}
// 注册
...
// 排除需要用户登录路由
server.use(/^(?!\/(auth|post|users)).*$/, (req, res, next) => {
  ...
  try {
    ...
    if (verifyTokenResult instanceof Error) {
      return res.status(401).json({ message: 'Token 未通知' })
    }

    next()
  } catch (err) {
    res.status(401).json({ message: 'Token 已撤销' })
  }
})
